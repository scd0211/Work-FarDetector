...
void Convert_160ch_binarybz2_to_Tree_auto(int run, int subrun) {
	...
	std::string filename2 = Form("/ghi/fs02/mlf/nu/JSNS2_RawData_dhlee_Far/wavedumpMB/Raw_bz2file/R%06d/jsns2_far.r%06d.f%05d.dat.part2.bz2",run , run, subrun);
...
#define FADC(board, ch, sample) \
	...
	auto process_file = [&](const std::string &fname) {
		...
		while (true) {
			...
			if (bytesRead == 0 && bzerror == BZ_STREAM_END) {
				// 정상 EOF
				break;
			}

			if (bytesRead != (int)sizeof(EventData)) {
				std::cerr << "Incomplete EventData read from " << fname
					<< " (bytesRead=" << bytesRead
					<< ", expected=" << sizeof(EventData)
					<< ", bzerror=" << bzerror << ")"
					<< std::endl;
				break;
			}

			if ((int)eventData.SAM != num_samples) {
				std::cerr << "ERROR: SAM mismatch in " << fname
					<< " (board=" << eventData.board
					<< ", ch=" << eventData.ch
					<< ", event=" << eventData.event_number
					<< ", TTT=" << eventData.TriggerTimeStamp
					<< ", SAM=" << eventData.SAM
			...
			int expectedWaveBytes = (int)(sizeof(uint16_t) * num_samples);
			bytesRead = BZ2_bzRead(&bzerror, bzFile, waveform.data(), expectedWaveBytes);

			if (bytesRead != expectedWaveBytes) {
				std::cerr << "Error reading waveform data from " << fname
					<< " (bytesRead=" << bytesRead
					<< ", expected=" << expectedWaveBytes
					<< ", bzerror=" << bzerror << ")"
					<< std::endl;
				break;
			}
			...
			if (board >= num_boards || ch >= num_channels) {
				std::cerr << "ERROR: Out-of-bounds board/channel in " << fname
					<< " (board=" << board
					<< ", ch=" << ch
					<< ", event=" << eventData.event_number
					<< ", TTT=" << eventData.TriggerTimeStamp
