void Draw_ME_WF(int runno, int subrunno)
{
	...
	for(int i=0; i<172; i++){
		sprintf(hname,"hPDF%02d",i);
		hPDF[i] = new TH3D(hname,hname,1,20,500,248,0,248,100,-0.4,1.1);
	}
	...
	for(int iEntry=0; iEntry<ntot; iEntry++){
		...
		for(int ipmt=0; ipmt<172; ipmt++){
			...
			for(int isam=0; isam<250; isam++){
				...
				Q_ratio = wave_align[ipmt][isam]/wave_align[ipmt][50];
