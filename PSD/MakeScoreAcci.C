...
void MakeScoreAcci(int ptag, int runno, int subrunno,const char *ME_PDFname, const char *FN_PDFname, const char *indir, const char *outdir)
{
...
//	double e60=8000;
	...
	ntp->Branch("pedestal",ped,"pedestal[172]/D");
	ntp->Branch("pedestal_var",ped_var,"pedestal_var[172]/D");
...
//			if(v3.Mag()>0.6) continue;
		...
		for(int ipmt=0; ipmt<172; ipmt++){
			...
			ped_var[ipmt]=pedestal_var[ipmt];
			Score_1Ds[ipmt] = 0;
			...
			for(int isam=0; isam<250; isam++){
				...
				if( P_ME>0. && P_FN>0.){
					Score_1D[ipmt][isam] =  log(P_ME) - log(P_FN); // 1D PSD score for each PMT and each FADC timing
					...
					PSDscore_1D +=  log(P_ME) - log(P_FN); // 1D PSD score
