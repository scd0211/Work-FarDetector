...
void DrawME()
{
	...
	chain->Add(Form("tree/ME/R000549.root"));
	chain->LoadTree(0);
	...
	TH1D * hPromptQ = new TH1D("hPromptQ","hPromptQ",100,160000,500000);
	TH1D * hDelayedQ = new TH1D("hDelayedQ","hDelayedQ",100,30000,80000);
...
//	gStyle->SetOptFit(1);
	...
	TF1 * f1 = new TF1("f1",func_exp1,2,10,2);
...
//	f1->SetLineColor(2);
	...
	hPromptQ->Draw("hist");
	hPromptQ->SetTitle("");
	hPromptQ->GetXaxis()->SetTitle("Flux");
	...
	hDelayedQ->Draw("hist");
	hDelayedQ->SetTitle("");
	hDelayedQ->GetXaxis()->SetTitle("Flux");
