...
void MakeAcciGamma(int run=549, int subrun=0, const char *outdir="./")
{
	...
	chain->Add(Form("/home/mlf/cdshin/Tape/FarDetector/10FADC_differential_trigger/R%06d/Reco.tree_jsns2_far.r%06d.f%05d.root",run,run,subrun));
	chain->LoadTree(0);
	...
	for (int iEntry = 0; iEntry < nTot; iEntry++) {
		...
		for(int ievt=0; ievt<Num_Events; ievt++){
			...
			if(!FNprompt && !veto_tag && recof>30000 && recof<100000){
				...
				if(time_diff>100 && !veto_tag &&  recof>30000 && recof<100000){
					p_time = trigtime+evttime;
					//promptQ = EventTotalCharge->at(ievt);
					promptQ = recof;
					p_vertx = v3.X();
				...
				else if(time_diff<100 && !veto_tag && recof>10000 && recof<20000){
					//delayedQ = EventTotalCharge->at(ievt);
					delayedQ = recof;
					d_vertx = v3.X();
					d_verty = v3.Y();
					d_vertz = v3.Z();
					cout<<TriggerID<<" "<<ievt<<endl;
					ntp->Fill();
					FNprompt=false;
				}
