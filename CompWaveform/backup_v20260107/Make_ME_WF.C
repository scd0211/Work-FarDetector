...
void Make_ME_WF(int run=422, int subrun=0, const char *outdir="tree/ME")
{
	...
	chain->Add(Form("/home/mlf/cdshin/Tape/FarDetector/FarREC_8FADC_diffREC/Reco.tree_jsns2_far.r%06d.f%05d.root",run,subrun));
	...
	for (int iEntry = 0; iEntry < nTot; iEntry++) {
		...
		for(int ievt=0; ievt<Num_Events; ievt++){
			...
			for (int ipmt = 0; ipmt < inner_pmt; ipmt++) {
				...
				//if(ipmt==0 && pedestal[ipmt]==0)	cout<<iEntry<<" "<<ievt<<" "<<ipmt<<" "<<pedestal[ipmt]<<" "<<PMT_Charge->at(ipmt + ievt * nPMT)<<endl;
