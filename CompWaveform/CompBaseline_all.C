{
	...
	//TFile * a1 = new TFile("hist/Baseline/BeforeN2.root");	
	...
	TGraphErrors * gr_Q = new TGraphErrors();
	int npoint=0;
	...
	TH1D * h3 = (TH1D*)a1->Get(Form("hQ1_%03d",ipmt));
	TH1D * h4 = (TH1D*)a1->Get(Form("hQ2_%03d",ipmt));
...
//	h1->GetYaxis()->SetTitle("a.u.");
	...
	float mean2 = h2->GetMean();

	//gr_diff->SetPoint(npoint,ipmt,center1-center);
	gr_diff->SetPoint(npoint,ipmt,mean2-mean1);
	gr_Q->SetPoint(npoint,ipmt,h3->GetMean());
	npoint++;
	...
	gr_diff->GetYaxis()->SetTitle("ped diff (ME-muon)");
	...
	gr_Q->Draw("ap");
	gr_Q->SetTitle("");
	gr_Q->GetXaxis()->SetTitle("PMT ID");
	gr_Q->GetYaxis()->SetTitle("PMT charge (muon) [p.e.]");
	gr_Q->SetMarkerStyle(20);
	gr_Q->SetMarkerColor(4);
	c4->Print("png/muon_charge.png");



	//TFile * fout = new TFile("baseline_cut_BeforeN2.root","RECREATE");
	TFile * fout = new TFile("baseline_cut_AfterN2.root","RECREATE");
	hPed_m->Write();
	hPed_r->Write();
	fout->Close();
