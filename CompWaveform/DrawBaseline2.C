void DrawBaseline2()
{

	TFile * fPED = new TFile("baseline_cut.root");
	TH1D * hPEDm = (TH1D*)fPED->Get("hPed_m");
	TH1D * hPEDr = (TH1D*)fPED->Get("hPed_r");

	FILE * fmask = fopen("/home/mlf/cdshin/FarDetector/EventReco/10FADC_20us_develop_differential/dat/PMT_masking_list_v20250702_8FADC.txt","r");

	int MaskTag[172];

	int pmtid, mtag;
	for(int ipmt=0; ipmt<172; ipmt++){
		fscanf(fmask,"%d %d",&pmtid, &mtag);

		if(ipmt!=pmtid){
			cout<<"mismatch masked pmt"<<endl;
			return;
		}
		MaskTag[ipmt]=mtag;
	}


	TH1D * hNpmt1 = new TH1D("hNpmt1","hNpmt1",120,0,120);

	double wave[172][250]={0};
	double wave_align[172][250]={0};
	double pedestal1[220];
	double pedestal2[220];
	double pedestal_var[220];
	double PMTCharge[172];
	double CorPMTCharge[172];

	double promptQ;
	double delayedQ;
	double time_diff;
	double p_vertx;
	double p_verty;
	double p_vertz;
	double d_vertx;
	double d_verty;
	double d_vertz;

	double p_vertx1;
	double p_verty1;
	double p_vertz1;
	double d_vertx1;
	double d_verty1;
	double d_vertz1;

	double p_vertx2;
	double p_verty2;
	double p_vertz2;
	double d_vertx2;
	double d_verty2;
	double d_vertz2;

	double p_vertx3;
	double p_verty3;
	double p_vertz3;
	double d_vertx3;
	double d_verty3;
	double d_vertz3;

	double nevt=0;


	TChain* chain = new TChain("tree");
	chain->Add(Form("tree/Baseline/ME_R000454.root"));
	//chain->Add(Form("tree/Baseline/ME_BeforeN2.root"));
	chain->LoadTree(0);
	chain->SetBranchAddress("promptQ", &promptQ);
	chain->SetBranchAddress("delayedQ", &delayedQ);
	chain->SetBranchAddress("p_vertx", &p_vertx);
	chain->SetBranchAddress("p_verty", &p_verty);
	chain->SetBranchAddress("p_vertz", &p_vertz);
	chain->SetBranchAddress("d_vertx", &d_vertx);
	chain->SetBranchAddress("d_verty", &d_verty);
	chain->SetBranchAddress("d_vertz", &d_vertz);
	chain->SetBranchAddress("time_diff", &time_diff);
	chain->SetBranchAddress("PMTCharge",PMTCharge);
	chain->SetBranchAddress("CorPMTCharge",CorPMTCharge);
	chain->SetBranchAddress("pedestal1",pedestal1);
	chain->SetBranchAddress("pedestal2",pedestal2);

	TH1D * hPed1[172];
	TH1D * hPed2[172];

	TH1D * hQ1[172];
	TH1D * hQ2[172];

	TH2D * hQ_diff[172];
 
	char hname[255];

	for(int ipmt=0; ipmt<172; ipmt++){
		sprintf(hname,"hPed1_%03d",ipmt);
		hPed1[ipmt]= new TH1D(hname,hname,10000,13000,15000);	
		sprintf(hname,"hPed2_%03d",ipmt);
		hPed2[ipmt]= new TH1D(hname,hname,10000,13000,15000);	

		sprintf(hname,"hQ1_%03d",ipmt);
		hQ1[ipmt]= new TH1D(hname,hname,5000,0,5000);	
		sprintf(hname,"hQ2_%03d",ipmt);
		hQ2[ipmt]= new TH1D(hname,hname,5000,0,5000);	

		sprintf(hname,"hQ_diff_%03d",ipmt);
		hQ_diff[ipmt]= new TH2D(hname,hname,5000,0,5000,200,-100,100);	
	}
	

	TVector3 v1;
	TVector3 v2;
	TVector3 v3;

	TVector3 v1_1;
	TVector3 v2_1;
	TVector3 v3_1;

	TVector3 v1_2;
	TVector3 v2_2;
	TVector3 v3_2;

	TVector3 v1_3;
	TVector3 v2_3;
	TVector3 v3_3;

	int ntot=chain->GetEntries();
	for(int iEntry=0; iEntry<ntot; iEntry++){

		if(iEntry%10000==0) cout<<iEntry<<"/"<<ntot<<" events"<<endl;
		chain->GetEntry(iEntry);

		v1.SetXYZ(p_vertx,p_verty,p_vertz);
		v2.SetXYZ(d_vertx,d_verty,d_vertz);

		v3 = v2-v1;
		if( !( v2.Perp()<1.0 && fabs(v2.Z())<1.0 ) ) continue;

		//if(promptQ<10000) continue;
		if(promptQ<9000) continue;
		if(delayedQ<2000) continue;
		if(delayedQ>7000) continue;
		if(time_diff<2) continue;

		for(int ipmt=0; ipmt<172; ipmt++){

			if(PMTCharge[ipmt]==0) continue;
//			if(PMTCharge[ipmt]>500) continue;
//			if(pedestal_var[ipmt]>10) continue;

				hPed1[ipmt]->Fill(pedestal1[ipmt]);
				hPed2[ipmt]->Fill(pedestal2[ipmt]);
				hQ1[ipmt]->Fill(CorPMTCharge[ipmt]);
				hQ2[ipmt]->Fill(PMTCharge[ipmt]);

				hQ_diff[ipmt]->Fill(CorPMTCharge[ipmt],pedestal2[ipmt]-pedestal1[ipmt]);
		}

		int n_High_PED_PMT=0;

		for(int iPMT=0; iPMT<172; iPMT++){
			if(iPMT==49) continue;
			if(iPMT==105) continue;
			if(MaskTag[iPMT]==1) continue;

			float PEDcut = hPEDm->GetBinContent(iPMT+1) + 3.5*hPEDr->GetBinContent(iPMT+1);
			if( pedestal1[iPMT] > PEDcut ) n_High_PED_PMT++;
		}

		hNpmt1->Fill(n_High_PED_PMT);	



		nevt++;
	}


	gSystem->mkdir(Form("hist/Baseline"), true);
	TFile * fout = new TFile(Form("hist/Baseline/AfterN2_npmt.root"),"RECREATE");

	hNpmt1->Write();

	fout->Close();


}
