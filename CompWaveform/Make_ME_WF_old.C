...
void Make_ME_WF_old(int run=422, int subrun=0, const char *outdir="tree/ME")
{
	...
	ntp->Branch("PMTCharge",pmtQ2,"PMTCharge[172]/D");
	ntp->Branch("CorPMTCharge",pmtQ1,"CorPMTCharge[172]/D");
	ntp->Branch("pedestal",pedestal,"pedestal[220]/D");
	ntp->Branch("pedestal_var",pedestal_var,"pedestal_var[220]/D");
	ntp->Branch("pedestal1",pedestal1,"pedestal1[220]/D");
	ntp->Branch("pedestal_var1",pedestal_var1,"pedestal_var1[220]/D");
	ntp->Branch("pedestal2",pedestal2,"pedestal2[220]/D");
	ntp->Branch("pedestal_var2",pedestal_var2,"pedestal_var2[220]/D");
		...
		for(int ievt=0; ievt<Num_Events; ievt++){
			...
			if(!FNprompt && veto_tag){
				...
				if(time_diff>10 && veto_tag){
					...
					for (int ipmt = 0; ipmt < inner_pmt; ipmt++){
						pmtQ2[ipmt]=pmtQ[ipmt];
				 		pedestal2[ipmt]=pedestal[ipmt];
				 		pedestal_var2[ipmt]=pedestal_var[ipmt];
					}

					for (int ipmt = 0; ipmt < inner_pmt; ipmt++){
				 		pedestal[ipmt]=pedestal2[ipmt];
				 		pedestal_var[ipmt]=pedestal_var2[ipmt];
					}

					AlignWaveforms(chain1, TrigID,ievt, nsample, EventTime->at(ievt));
					ntp->Fill();
					FNprompt=false;
				//	for(int isam=0; isam<250; isam++){
				//		hWF->SetBinContent(isam+1,wave_align[0][isam]);
				//	}
				}
